...
public class CalendarBean {
    ...
    private CalendarBean(int month, int year) {
        days = new int[6][7];
        numberOfWeeks = 0;
        this.month = month;
        this.year = year;
    ...
    public static CalendarBean getMonth(int month, int year) {
        String key = String.valueOf((new StringBuffer(String.valueOf(month))).append("/").append(year));
        if (months.containsKey(key)) {
            return (CalendarBean) months.get(key);
        } else {
            CalendarBean newMonth = new CalendarBean(month, year);
